...
export function InventoryProvider({ children }: { children: ReactNode }) {
  ...
  const updateItem = (id: string, updatedFields: Partial<Item>) => {
    ...
    // If category changed, update category item counts
    if (item && updatedFields.category && updatedFields.category !== item.category) {
      updateCategoryItemCount(item.category, -1) // Decrease old category count
      updateCategoryItemCount(updatedFields.category, 1) // Increase new category count
    }
  ...
  const updateCategory = (id: string, name: string) => {
    ...
    if (category && name !== category.name) {
      ...
      setItems((prev) => prev.map((item) => (item.category === category.name ? { ...item, category: name } : item)))

      // Update the category name
      setCategories((prev) => prev.map((cat) => (cat.id === id ? { ...cat, name: name } : cat)))
    }
  }
